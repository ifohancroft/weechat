 ...
 *
 * If force == 1, all used items are refreshe ((according to option
 * buflist.look.items).
...
buflist_bar_item_update (int force)
{
    ...
    if (force || weechat_config_boolean (buflist_config_look_enabled))
    {
        num_items = (force == 2) ?
            ...
            weechat_config_integer (buflist_config_look_items);
                             ...
                             struct t_hashtable *extra_info)
{
    ...
    if (item_index + 1 > weechat_config_integer (buflist_config_look_items))
        return NULL;
